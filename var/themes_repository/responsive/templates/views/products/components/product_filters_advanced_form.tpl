...
<table class="ty-filters-search">
{foreach from=$splitted_filter item="filters_row" name="filters_row"}
<tr>
{foreach from=$filters_row item="filter"}
    <th class="ty-filters-search__title">{$filter.filter|default:$filter.description|default:""}</th>
{/foreach}
</tr>
<tr class="ty-valign-top{if ($splitted_filter|sizeof > 1) && $smarty.foreach.filters_row.first} ty-filters-search__delim{/if}">
{foreach from=$filters_row item="filter"}
    <td class="ty-filters-search__item" style="width: 33%">
...
<div class="buttons-container">
    ...
    &nbsp;{__("or")}&nbsp;&nbsp;<a class="text-button cm-reset-link">{__("reset_filter")}</a>
</div>
